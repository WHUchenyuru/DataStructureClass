...
typedef struct
{
    int col[MaxSize];   //col[i]存放第id个皇后的位置
    int top;            //栈顶指针
}StackType;             //声明顺序栈类型
...
void queen(int n)                           //求解n皇后问题
{
    ...
    while(St.top != 0)              //栈不为空时循环
    {
        k = St.top;                 //试探栈顶的第k个皇后
        find = false;               //尚未找到第k个皇后的位置，find置为假
        ...
        if(find)                    //在第k行找到一个放皇后的位置(k,j)
        {
            if(k == n)              //若所有皇后均放好，输出一个解
                dispasolution(St);
            else                    //还有皇后未放时，将第k+1个皇后入栈
            {
                St.top++;
                St.col[St.top] = 0; //新进栈的皇后从低到0列开始试探
            }
        }
        else                        //若第k个皇后没有合适位置，回测
            St.top--;               //即将第k个皇后退栈
    }
}
